...
public class FafurionHard extends Quest
{
	// NPCs
	private static int			DEVICE			= 70000;
	private static int			TELEPORTER		= 70004;
	private static int			TELEPORTER2		= 70003;
	...
	{
		55550, 55517, 55516, 55515, 55514, 55513, 55512, 55510, 55509, 55508, 55507, 55506, 55505, 55504, 55503, 55502, 55501, 55600, 55628, 56327, 55603, 55627, 55628,
	};
	private static String		qn				= "FafurionHard";
	private static final int	INSTANCEID		= 2005;
	private static boolean		debug			= false;
	private static int			levelReq		= Config.FAFURION_HARD_LEVELS;
	private static int			pvpReq			= Config.FAFURION_HARD_PVPS;																								// Previously 50
	private static int			healerPvpReq	= Config.FAFURION_HARD_SUPPORT_PVPS;																								// Custom
	...
	public FafurionHard(int questId, String name, String descr)
	{
	...
	{
		new FafurionHard(-1, qn, "instances");
	}
		...
		if (st != null)
		{
			if (npcId == DEVICE)
			{
				...
				if (fafWorld == null)
				{
					...
					teleto.y = 249484;
					teleto.z = -3393;
					enterInstance(player, "FafurionHard.xml", teleto);
				}
				else
				{
			...
			else if (npcId == TELEPORTER)
			{
				...
				if (player.getParty() == null)
				{
					switch (fafWorld.getStage())
					{
						...
						case 4:
						case 5:
							player.sendPacket(new ExShowScreenMessage("Stage: " + 2, 6000));
							break;
						case 6:
						case 7:
							player.sendPacket(new ExShowScreenMessage("Stage: " + 3, 6000));
							break;
					}
...
//					player.sendPacket(new ExShowScreenMessage("Stage: " + String.valueOf(fafWorld.stage), 6000));
				...
				else
				{
					...
					for (L2PcInstance ptm : player.getParty().getPartyMembers())
					{
						...
						ptm.sendPacket(new ExShowScreenMessage("Stage: " + String.valueOf(stage), 6000));
					}
				}
			}
	...
	public void spawn2ndMobs(FafurionWorld world, L2PcInstance player) // Temple OutSide STAGE 2 XONTROS FIGHT
	{
		if (world.getStage() == 1)
		{
			addSpawn(55603, 187215, 255287, -3322, 43110, false, 0, false, world.instanceId); // Fafurion's Shield Master
			world.incLiveMobs();
			world.incStage();
		}
	}
	...
	public void spawn4thMobs(FafurionWorld world, L2PcInstance player) // Nest Cave Inside STAGE 4 Boss Fight
	{
		if (world.getStage() == 3)
		{
			addSpawn(55627, 191017, 257058, -3318, 28544, false, 0, false, world.instanceId); // Water Spirit
			world.incLiveMobs();
			world.incStage();
		}
	}
	...
	public void spawn6thMobs(FafurionWorld world, L2PcInstance player) // Fafurion Cave Inside STAGE 6 Boss Fight
	{
		if (world.getStage() == 5)
		{
			addSpawn(55628, 180023, 217435, -14304, 31481, false, 0, false, world.instanceId); // Spirit King
			world.incLiveMobs();
			world.incStage();
		}
	}
	...
	public void spawn7thMobs(FafurionWorld world, L2PcInstance player) // Fafurion BOSS
	{
		if (world.getStage() == 6)
		{
			addSpawn(55600, 180767, 210605, -14812, 14948, false, 0, false, world.instanceId); // Fafurion
			world.incLiveMobs();
			world.incStage();
		}
	}
	...
	public L2Npc spawnGate(int npcId, int x, int y, int z, int heading, boolean randomOffset, int despawnDelay, boolean isSummonSpawn, int instanceId)
	{
		...
		try
		{
			...
			{
				_log.severe(qn + "Quest.java addSpawn() called a null NPC to be spawned w/ ID: " + npcId);
			}
		}
		catch (Exception e1)
		{
