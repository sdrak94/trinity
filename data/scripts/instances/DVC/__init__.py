#DVC Script by Psychokiller1888
...
#DVC Levels   23          26          33          36          43          46          53          56          63          66          73
...
def checkCondition(player,reuse,minLevel,maxLevel):
	...
	if not party:
		...
		return False
	#check size of the party, max 6 for entering DVC
	...
	for partyMember in party.getPartyMembers().toArray():
		if partyMember.getLevel() < minLevel or partyMember.getLevel() > maxLevel:
			player.sendPacket(SystemMessage.sendString("You and your party mates must be between level " + str(minLevel) + " and level " + str(maxLevel) + " to enter this DVC."))
			partyMember.sendPacket(SystemMessage.sendString("You must be between level " + str(minLevel) + " and level " + str(maxLevel) + " to enter this DVC."))
			return False
		st = partyMember.getQuestState(qn)
		if st:
			LastEntry = st.getInt("LastEntry")
			if currentTime < LastEntry + reuse:
				player.sendPacket(SystemMessage.sendString("One of your party member still has to wait for re-access DVC"))
				partyMember.sendPacket(SystemMessage.sendString("You have to wait at least 24 hours between each time you enter DVC"))
				return False
	return True
...
def enterInstance(self,player,teleto,KamaInfo):
	...
	if checkDistance(player):
		player.sendPacket(SystemMessage.sendString("Please regroup your party before joining DVC."))
		return 0
	...
	if not self.worlds.has_key(instanceId):
		...
		print "DVC: started " + template + " Instance: " +str(instanceId) + " created by player: " + str(player.getName())
	# Teleport players
...
class DVC(JQuest):
	...
	def onAdvEvent (self,event,npc,player):
		...
		if event == "finishDVC" :
			...
			KamaInfo = ["DVC-23.xml",86400,18554,18,28,-57109,-219871,-8117]
		elif event == "lvl26" :
			KamaInfo = ["DVC-26.xml",86400,18555,21,31,-55556,-206144,-8117]
		elif event == "lvl33" :
			KamaInfo = ["DVC-33.xml",86400,18558,28,38,-55492,-206143,-8117]
		elif event == "lvl36" :
			KamaInfo = ["DVC-36.xml",86400,18559,31,41,-41257,-213143,-8117]
		elif event == "lvl43" :
			KamaInfo = ["DVC-43.xml",86400,18562,38,48,-49802,-206141,-8117]
		elif event == "lvl46" :
			KamaInfo = ["DVC-46.xml",86400,18564,41,51,-41184,-213144,-8117]
		elif event == "lvl53" :
			KamaInfo = ["DVC-53.xml",86400,18566,48,58,-41201,-219859,-8117]
		elif event == "lvl56" :
			KamaInfo = ["DVC-56.xml",86400,18568,51,61,-57102,-206143,-8117]
		elif event == "lvl63" :
			KamaInfo = ["DVC-63.xml",86400,18571,58,68,-57116,-219857,-8117]
		elif event == "lvl66" :
			KamaInfo = ["DVC-66.xml",86400,18573,61,71,-41228,-219860,-8117]
		elif event == "lvl73" :
			KamaInfo = ["DVC-73.xml",86400,18577,68,78,-55823,-212935,-8071]
		tele = PyObject()
		tele.x = KamaInfo[5]
		tele.y = KamaInfo[6]
		tele.z = KamaInfo[7]
	...
	def onKill(self,npc,player,isPet):
		...
		for member in playerList.toArray():
			member = L2World.getInstance().findPlayer(member)
			saveEntry(member)
			member.sendPacket(SystemMessage.sendString("You will be moved out of DVC in 5 minutes"))
		self.startQuestTimer("finishDVC",300000,npc,player)
		return

QUEST = DVC(-1, qn, "instances")
