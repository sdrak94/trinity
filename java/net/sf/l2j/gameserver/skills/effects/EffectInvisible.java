...
{
public EffectInvisible(Env env, EffectTemplate template, Boolean ignoreBoost)
{
	super(env, template, ignoreBoost);
}

public EffectInvisible(Env env, L2Effect effect)
{
	super(env, effect);
}
...
public boolean onStart()
{
	...
	for (L2Character nigga : getEffected().getKnownList().getKnownCharacters())
	{
		if (nigga != null && nigga != getEffected() && nigga.getKnownList() != null && nigga.getKnownList().knowsObject(getEffected()))
		{
			...
			{
				if (getEffected().isAPC() || !nigga.canSeeInvisiblePeople() || (activeChar != null && activeChar.isGMReally() && activeChar.getAccessLevel().getLevel() > ((L2PcInstance)nigga).getAccessLevel().getLevel()))
