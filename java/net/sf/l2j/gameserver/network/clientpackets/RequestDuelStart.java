...
protected void runImpl()
{
	...
	if (activeChar.getAccessLevel().getLevel() < targetChar.getAccessLevel().getLevel())
	{
		if (targetChar.isInvisible() || targetChar.getBlockList().isBlockAll())
		{
			activeChar.sendPacket(new SystemMessage(SystemMessageId.THERE_IS_NO_OPPONENT_TO_RECEIVE_YOUR_CHALLENGE_FOR_A_DUEL));
			return;
		}
		...
		{
			activeChar.sendMessage("Do not try to duel those that can own you.");
			return;
		}
	}
	if (activeChar.getPvpFlag() != 0 || targetChar.getPvpFlag() != 0)
	{
		activeChar.sendPacket(new SystemMessage(SystemMessageId.YOU_ARE_UNABLE_TO_REQUEST_A_DUEL_AT_THIS_TIME));
		return;
	}
	if (BlockList.isBlocked(targetChar, activeChar))
	{
		activeChar.sendPacket(new SystemMessage(SystemMessageId.THERE_IS_NO_OPPONENT_TO_RECEIVE_YOUR_CHALLENGE_FOR_A_DUEL));
		return;
	}
	if (targetChar.getTradeRefusal() && !activeChar.isGM())
	{
	...
	{
		activeChar.sendMessage("You must be level 83 or higher to duel");
		return;
	}
	if (targetChar.getLevel() < 83)
	{
		activeChar.sendMessage("Target must be level 83 or higher to duel");
		return;
	}
	...
	else if (!activeChar.isInsideRadius(targetChar, 250, false, false))
	{
		...
		msg.addString(targetChar.getDisplayName());
	...
	if (_partyDuel == 1)
	{
		...
		if (!partyLeader.isProcessingRequest())
		{
			...
			partyLeader.sendPacket(new ExDuelAskStart(activeChar.getDisplayName(), _partyDuel));
	...
	else // 1vs1 duel
	{
		if (!targetChar.isProcessingRequest())
		{
			...
			targetChar.sendPacket(new ExDuelAskStart(activeChar.getDisplayName(), _partyDuel));
