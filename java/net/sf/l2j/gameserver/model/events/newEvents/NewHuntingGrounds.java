...
public class NewHuntingGrounds
{
	protected static final Logger			_log					= Logger.getLogger(NewHuntingGrounds.class.getName());
	public static String					_eventName				= "";
	public static String					_dispEventName			= "Hunting Grounds";
	...
	public static void kickPlayerFromHG(L2PcInstance playerToKick)
	{
		...
		else if (_started || _teleport)
		{
			...
			if (playerToKick.isOnline() != 0)
			{
				...
				playerToKick.sendMessage("You have been kicked from the Hunting Grounds.");
	...
	public static boolean startAutoJoin()
	{
		...
		AnnounceToPlayers(true, "Registration for Hunting Grounds event: " + _eventName + " has been opened");
	...
	private static void spawnEventNpc()
	{
		...
		{
			_log.severe("Hunting Grounds Engine[spawnEventNpc(exception: " + e.getMessage());
		}
	}
	...
	public static void unTransformAll()
	{
	...
	public static void preparePlayers()
	{
		for (L2PcInstance player : _players)
		{
			...
			{
				if (player.getInventory().getItemByItemId(6368) == null)
				{
					player.addItem("HG", 6368, 1, null, false, 0);
				}
				L2ItemInstance item = player.getInventory().getItemByItemId(6368);
				if (!player.getInventory().getItemByItemId(6368).isEquipped())
				{
			...
			{
				if (player.getInventory().getItemByItemId(289) == null)
				{
					player.addItem("HG", 289, 1, null, false, 0);
				}
				L2ItemInstance item = player.getInventory().getItemByItemId(289);
				if (!player.getInventory().getItemByItemId(289).isEquipped())
				{
					player.useEquippableItem(item, true, true);
				}
			}
	...
	public static boolean teleportAutoStart()
	{
		...
		_joining = false;
		AnnounceToPlayers(false, _eventName + "(Hunting Grounds): Teleport to team spot in 20 seconds!");
		setUserData();
		spawnBuffers();
		spawnFences();
		setPara(true);
			...
			public void run()
			{
				for (L2PcInstance player : _players)
				{
					if (player != null)
					{
						...
						player.teleToLocation(getRndTeamLoc(player._teamNameHG), false);
						// player.teleToLocation(_teamsX.get(_teams.indexOf(player._teamNameHG)), _teamsY.get(_teams.indexOf(player._teamNameHG)), _teamsZ.get(_teams.indexOf(player._teamNameHG)));
	...
	public static boolean startAutoEvent()
	{
		...
		setPara(false);
		// sit();
		AnnounceToPlayers(false, _eventName + "(Hunting Grounds): Started. Go to kill your enemies!");
		_started = true;
		for (L2PcInstance player : _players)
		{
			player.broadcastUserInfo();
			...
			if (_joining || _started || _teleport)
			{
				switch (seconds)
				{
					case 3600: // 1 hour left
						if (_joining)
						{
							AnnounceToPlayers(true, "(Hunting Grounds): Joinable in " + _joiningLocationName + "!");
							AnnounceToPlayers(true, "HG Event: " + seconds / 60 / 60 + " hour(s) till registration ends!");
						}
						else if (_started)
							AnnounceToPlayers(false, "HG Event: " + seconds / 60 / 60 + " hour(s) till event ends!");
						break;
					...
					case 60: // 1 minute left
						if (_joining)
						{
							removeOfflinePlayers();
							AnnounceToPlayers(true, "(Hunting Grounds): Joinable in " + _joiningLocationName + "!");
							AnnounceToPlayers(true, "Hunting Grounds Event: " + seconds / 60 + " minute(s) till registration ends!");
						}
						else if (_started)
							AnnounceToPlayers(false, "Hunting Grounds Event: " + seconds / 60 + " minute(s) till event ends!");
						break;
					...
					case 1: // 1 seconds left
						if (_joining)
							AnnounceToPlayers(true, "Hunting Grounds Event: " + seconds + " second(s) till registration ends!");
						else if (_teleport)
							AnnounceToPlayers(false, "Hunting Grounds Event: " + seconds + " seconds(s) till fight starts!");
						else if (_started)
							AnnounceToPlayers(false, "Hunting Grounds Event: " + seconds + " second(s) till event ends!");
						break;
				}
			}
	...
	private static void checkPlayers()
	{
		...
		{
			if (p.getSkillLevel(35100) != 1 || p.getSkillLevel(35100) == 0)
			{
				p.addSkill(35100);
			}
		}
	...
	{
		_players.get(index)._teamNameHG = _teams.get(TeamTo);
	...
	public static void finishEvent()
	{
		try
		{
			...
			synchronized (topkiller)
			{
				...
				{
					if (player._countHGkills > _top1Kills || (player._countHGkills >= _top1Kills && player._countHGdies < topkiller[0]._countHGdies))
					{
						topkiller[0] = player;
						_top1Kills = player._countHGkills;
					}
				}
				for (L2PcInstance player : plrs)
				{
					if (player != topkiller[0] && !player._teamNameHG.equals(topkiller[0]._teamNameHG) && (player._countHGkills > _top2Kills || (player._countHGkills >= _top2Kills && player._countHGdies < topkiller[1]._countHGdies)))
					{
						topkiller[1] = player;
						_top2Kills = player._countHGkills;
					}
				}
				for (L2PcInstance player : plrs)
				{
					if (player != null  && player._countHGkills > 0 && player != topkiller[0] && player != topkiller[1] && !player._teamNameHG.equals(topkiller[0]._teamNameHG) && !player._teamNameHG.equals(topkiller[1]._teamNameHG) && (player._countHGkills > _top3Kills || (player._countHGkills >= _top3Kills && player._countHGdies < topkiller[2]._countHGdies)))
					{
						topkiller[2] = player;
						_top3Kills = player._countHGkills;
					}
				}
			}
			// AnnounceToPlayers(true, _eventName + "(Hunting Grounds): " + _topTeam + "'s win the match! " + _topKills + " kills.");
			...
			if (topkiller[0] != null)
			{
				...
				AnnounceToPlayers(true, topkiller[0].getName() + " - " + topkiller[0]._teamNameHG + " team - " + _top1Kills + " kills - " + topkiller[0]._countHGdies + " deaths");
			}
			if (topkiller[1] != null)
				AnnounceToPlayers(true, topkiller[1].getName() + " - " + topkiller[1]._teamNameHG + " team - " + _top2Kills + " kills - " + topkiller[1]._countHGdies + " deaths");
			if (topkiller[2] != null)
				AnnounceToPlayers(true, topkiller[2].getName() + " - " + topkiller[2]._teamNameHG + " team - " + _top3Kills + " kills - " + topkiller[2]._countHGdies + " deaths");
			teleportFinish();
		}
		catch (Throwable e)
		{
	...
	public static void processTopTeam()
	{
		...
		{ // 2 loosing teams
			...
			AnnounceToPlayers(true, _eventName + "(Hunting Grounds): Match ended as Draw!");
	...
	public static void rewardTeam(String teamName, boolean tie)
	{
		...
		for (L2PcInstance player : _players)
		{
			...
			try
			{
				if (player != null && player.isOnline() == 1 && player._inEventHG)
				{
					if (player._countHGkills > 4)
					{
						if (player._teamNameHG.equals(teamName))
						{
							...
							if (Config.L2JMOD_ACHIEVEMENT_SYSTEM)
							{
								player.getCounters().hgWon++;
								player.getCounters().hgPlayed++;
							}
	...
	public static void abortEvent()
	{
		if (_joining && !_teleport && !_started)
		{
			unspawnEventNpc();
			_joining = false;
			AnnounceToPlayers(true, _eventName + "(Hunting Grounds): Match aborted!");
			_joining = false;
			_teleport = false;
			_started = false;
			_sGrade = false;
	...
	public static boolean addPlayerOk(String teamName, L2PcInstance eventPlayer)
	{
		...
		{
			_log.warning("Hunting Grounds Engine exception: " + e.getMessage());
		}
	...
	public static void removePlayer(L2PcInstance player)
	{
		...
		kills = player._countHGkills;
		deaths = player._countHGdies;
		score = 0;
		EventStats.getInstance().parseStats(player, new PlayerStatsTemplate("Hunting Grounds", 1, wins, loses, kills, deaths, score));
		player._teamNameHG = "";
		player._countHGkills = 0;
		player._countHGdies = 0;
	...
	public static void cleanHG()
	{
		if (_cleaned)
			return;
		_log.info("Hunting Grounds : Cleaning players.");
		for (L2PcInstance player : _players)
		{
			if (player != null)
			{
		...
		_log.info("Hunting Grounds : Cleaning teams.");
		if (_teams != null)
		{
			for (String team : _teams)
			{
		...
		_log.info("Cleaning Hunting Grounds done.");
	...
	{
		AnnounceToPlayers(false, _eventName + "(Hunting Grounds): Teleport back to participation NPC in 20 seconds!");
		setPara(true);
		ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
		{
			public void run()
			{
				...
				_log.info("Hunting Grounds: Teleport finish done.");
				cleanHG();
			}
		}, 20000);
	}
	...
	public static void handleSkill(L2PcInstance p, boolean add)
	{
		if (add)
		{
			p.addSkill(SkillTable.getInstance().getInfo(35100, 1), false);
		}
		else
			p.removeSkill(35100);
	}
	...
	public static Comparator<L2PcInstance> comparePlayersScore = new Comparator<L2PcInstance>()
	{
		...
		{
			int score1 = p1._countHGkills;
			int score2 = p2._countHGkills;
			if (score1 == score2)
			{
				int deaths1 = p1._countHGdies;
				int deaths2 = p2._countHGdies;
			...
			public void run()
			{
				if (player._inEventHG && _started)
				{
					if (player.isDead())
					{
						player.setInstanceId(_sGrade ? 3 : FOS.SIEGE_EVENT_INSTANCE_ID);
						player.teleToLocation(getRndTeamLoc(player._teamNameHG), false);
						// player.teleToLocation(_teamsX.get(_teams.indexOf(player._teamNameHG)), _teamsY.get(_teams.indexOf(player._teamNameHG)), _teamsZ.get(_teams.indexOf(player._teamNameHG)), false);
						player.doRevive();
