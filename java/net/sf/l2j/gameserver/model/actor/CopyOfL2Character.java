 ...
 */
public abstract class CopyOfL2Character extends L2Object
{
	public static final Logger				_log						= Logger.getLogger(CopyOfL2Character.class.getName());
	// =========================================================
	// Data Field
	private List<CopyOfL2Character>				_attackByList;
	...
	private List<CopyOfL2Character>				_raidDamageAttackers;
	...
	public final void setInsideZone(final byte zone, final boolean state)
	{
		...
		else
		{
			...
			if (_zones[zone] < 0)
				_zones[zone] = 0;
			if (zone == CopyOfL2Character.ZONE_PVP && this instanceof L2PcInstance)
			{
				if (!isGM())
				{
	 ...
	 */
	public CopyOfL2Character(int objectId, L2CharTemplate template)
	{
		super(objectId);
		initCharStat();
		initCharStatus();
	 ...
	 */
	public void addAttackerToAttackByList(CopyOfL2Character player)
	{
	...
	public void addRaidAttackerToList(CopyOfL2Character player)
	{
	...
	final public int getShldRate(CopyOfL2Character attacker, L2Skill skill)
	{
	...
	public void broadcastStatusUpdate()
	{
		...
		{
			for (CopyOfL2Character temp : getStatus().getStatusListener())
			{
				try
				{
					temp.sendPacket(su);
	...
	{
		private final CopyOfL2Character	_char;
		private int					_angle;
		private int					_originalheading;
		
		public ActionTask(CopyOfL2Character activeChar, int angle, int heading)
		{
			_char = activeChar;
			_angle = angle;
		...
		public void run()
		{
			...
			try
			{
				Collection<CopyOfL2Character> list = _char.getKnownList().getKnownCharactersInRadius(_char.getPhysicalAttackRange());
				if (list != null && list.size() > 0)
				{
					FastList<CopyOfL2Character> attackables = new FastList<CopyOfL2Character>();
					for (CopyOfL2Character target : list)
					{
	 ...
	 */
	protected void doAttack(CopyOfL2Character target)
	{
	...
	public boolean canAttackDueToSoloMob(CopyOfL2Character target)
	{
		if (isGM())
			return true;
		if (target instanceof L2Attackable)
		{
			...
			{
				CopyOfL2Character otherGuy = attackable.getAI().getAttackTarget();
	...
	private boolean doAttackHitByBow(final Attack attack, CopyOfL2Character target, int sAtk, int reuse, boolean crossbow)
	{
		int damage1 = 0;
		byte shld1 = 0;
		boolean crit1 = false;
		...
		if (maxPierced >= 1 && this instanceof L2PcInstance)
		{
			...
			for (CopyOfL2Character chara : getKnownList().getKnownCharactersInRadius(distance))
			{
		...
		if (aoeRadius > 0)
		{
			...
			final int distance = (int) Math.max(1, Util.calculateDistance(this, target, true));
			for (CopyOfL2Character chara : getKnownList().getKnownCharactersInRadius(distance + aoeRadius))
			{
				if (chara == null || chara == this || chara == target)
					continue;
				if (isTargettingMob)
	...
	final private void doChainAttackHitByBow(CopyOfL2Character target, int sAtk, int pierceRating, int aoeRadius, float aoeDmgMod)
	{
		int maxPierced = 0;
		if (pierceRating > 0)
		{
		...
		if (aoeRadius > 0)
		{
			...
			byte count = 0;
			boolean pvpupdated = false;
			for (CopyOfL2Character chara : target.getKnownList().getKnownCharactersInRadius(aoeRadius))
			{
				if (chara == null || chara == this || chara == target)
					continue;
				if (isTargettingMob)
	...
	final private void doPierceAttackHitByBow(CopyOfL2Character target, int sAtk, int aoeRadius, float dmgMod)
	{
		int damage1 = 0;
		byte shld1 = 0;
		boolean crit1 = false;
	...
	private CopyOfL2Character findPossibleTank(CopyOfL2Character target)
	{
	...
	private boolean doAttackHitByDual(Attack attack, CopyOfL2Character target, int sAtk)
	{
		return doAttackHitByDual(attack, target, 100, sAtk, (byte) 1);
	}
	
	private boolean doAttackHitByDual(Attack attack, CopyOfL2Character target, double attackpercent, int sAtk, byte absNerf)
	{
		int damage1 = 0;
		int damage2 = 0;
		byte shld1 = 0;
	...
	private boolean doAOEHitbyDualWeapon(Attack attack, CopyOfL2Character target, int sAtk)
	{
		double angleChar;
		...
		for (CopyOfL2Character obj : getKnownList().getKnownCharactersInRadius((int) getStat().calcStat(Stats.POWER_ATTACK_RANGE, getTemplate().baseAtkRange, null, null)))
		{
			if (obj == this)
				continue;
			if (obj == target)
	...
	private boolean doAOEHitbySingleWeapon(Attack attack, CopyOfL2Character target, int sAtk)
	{
		double angleChar;
		final int maxAngleDiff = (int) getStat().calcStat(Stats.POWER_ATTACK_ANGLE, 120, null, null);
		if (maxAngleDiff < 355)
		...
		int attackcount = 0;
		final boolean isTargettingMob = (target instanceof L2Attackable);
		for (CopyOfL2Character obj : getKnownList().getKnownCharactersInRadius(maxRadius))
		{
			if (obj == this)
				continue;
			if (obj == target)
	...
	private boolean doAttackHitSimple(Attack attack, CopyOfL2Character target, int sAtk)
	{
		return doAttackHitSimple(attack, target, 100, sAtk, (byte) 1);
	}
	
	private boolean doAttackHitSimple(Attack attack, CopyOfL2Character target, double attackpercent, int sAtk, byte absNerf)
	{
		int damage1 = 0;
		byte shld1 = 0;
		boolean crit1 = false;
	...
	private void beginCast(L2Skill skill, boolean simultaneously)
	{
		...
		CopyOfL2Character[] targets = skill.getTargetList(this);
	...
	{
		if (skill.isPassive() || !(l2Object instanceof CopyOfL2Character) || !checkDoCastConditions(skill))
			return false;
	...
	public void startFusionSkill(CopyOfL2Character target, L2Skill skill)
	{
		if (skill.getSkillType() != L2SkillType.FUSION)
			return;
	 ...
	 */
	public boolean doDie(CopyOfL2Character killer)
	{
		...
		try
		{
			if (_fusionSkill != null)
				abortCast();
			for (CopyOfL2Character character : getKnownList().getKnownCharacters())
	...
	protected void calculateRewards(CopyOfL2Character killer)
	{}
	...
	public final List<CopyOfL2Character> getAttackByList()
	{
		if (_attackByList == null)
			_attackByList = new FastList<CopyOfL2Character>();
		return _attackByList;
	}
	
	public final List<CopyOfL2Character> getRaidAttackerByList()
	{
		if (_raidDamageAttackers == null)
			_raidDamageAttackers = new FastList<CopyOfL2Character>();
	...
	{
		CopyOfL2Character	chara;
		int			_skillId;
		boolean		_all;
		
		public shotDisplayTask(CopyOfL2Character cha, int id, boolean all)
		{
			chara = cha;
			_skillId = id;
			_all = all;
	...
	class HitTask implements Runnable
	{
		...
		public HitTask(CopyOfL2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld, byte absNerf)
		{
			_hitTarget = target;
			_damage = damage;
			_crit = crit;
		...
		public HitTask(CopyOfL2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld, final boolean pierced)
		{
			_hitTarget = target;
			_damage = damage;
			_crit = crit;
		...
		public HitTask(CopyOfL2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld, byte absNerf, boolean playSound)
		{
			_hitTarget = target;
			_damage = damage;
			_crit = crit;
	...
	class MagicUseTask implements Runnable
	{
		...
		public MagicUseTask(L2Object[] targets, L2Skill skill, int hitTime, int coolTime, boolean simultaneously, int shot)
		{
			...
			_count = 0;
			_shots = shot;
			if (_skill.getTargetType(CopyOfL2Character.this) == SkillTargetType.TARGET_AURA || _skill.getTargetType(CopyOfL2Character.this) == SkillTargetType.TARGET_FRONT_AURA || _skill.getTargetType(CopyOfL2Character.this) == SkillTargetType.TARGET_BEHIND_AURA)
			{
				x = getX();
				y = getY();
				z = getZ();
	...
	public class AIAccessor
	{
		 ...
		 * <BR>
		 */
		public CopyOfL2Character getActor()
		{
			return CopyOfL2Character.this;
		}
		...
		{
			CopyOfL2Character.this.stopMove(pos);
		}
		 ...
		 * <BR>
		 */
		public void doAttack(CopyOfL2Character target)
		{
			CopyOfL2Character.this.doAttack(target);
		}
		...
		public void doCast(L2Skill skill)
		{
			CopyOfL2Character.this.doCast(skill);
		}
	...
	public boolean updatePosition(int gameTicks)
	{
		...
		if (Config.GEODATA > 0 && Config.COORD_SYNCHRONIZE == 2 && !isFlying() && !isInsideZone(CopyOfL2Character.ZONE_WATER) && !m.disregardingGeodata && GameTimeController.getGameTicks() % 10 == 0 // once a second to reduce possible cpu load
	 ...
	 */
	protected void onHitTimer(CopyOfL2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld, final boolean pierced, byte absNerf, boolean playSound)
	{
	...
	public void doKnockback(final CopyOfL2Character target, final L2Skill skill)
	{
		if (target.isInvul())
			return;
		if (this instanceof L2PcInstance)
	...
	private FlyType getAttackKnockbackType(CopyOfL2Character target)
	{
	...
	public void breakAttack(CopyOfL2Character attacker)
	{
		if (isAttackingNow())
		{
	...
	public void breakCast(CopyOfL2Character attacker)
	{
	...
	public boolean isInsidePeaceZone(L2Object attacker, L2Object target)
	{
		...
		if (!isInFunEvent() || !attacker.isInFunEvent())
		{
			...
			return (isInsideZone(ZONE_PEACE) || ((CopyOfL2Character) attacker).isInsideZone(ZONE_PEACE));
		}
		return false;
	}
	 ...
	 */
	@SuppressWarnings("incomplete-switch")
	public int calculateTimeBetweenAttacks(CopyOfL2Character target, L2Weapon weapon)
	{
	...
	public int calculateReuseTime(CopyOfL2Character target, L2Weapon weapon)
	{
		if (weapon == null || isTransformed())
			return 0;
	...
	public final Map<Integer, L2Skill> getAllSkillsNew()
	{
		return _skills;
	}
	...
	public void onMagicLaunchedTimer(MagicUseTask mut)
	{
		...
		if (escapeRange > 0)
		{
			List<CopyOfL2Character> targetList = new FastList<CopyOfL2Character>();
			for (L2Object target : targets)
			{
				if (target instanceof CopyOfL2Character)
				{
					...
					if (skill.isOffensive() && !skill.isNeutral())
					{
						if (this instanceof L2PcInstance)
						{
							if (((CopyOfL2Character) target).isInsidePeaceZone((L2PcInstance) this))
								continue;
						}
						else
						{
							if (((CopyOfL2Character) target).isInsidePeaceZone(this, target))
								continue;
						}
					}
					targetList.add((CopyOfL2Character) target);
				}
			}
			if (targetList.isEmpty())
			{
				...
				targets = targetList.toArray(new CopyOfL2Character[targetList.size()]);
		}
	...
	public void onMagicHitTimer(MagicUseTask mut)
	{
		...
		if (mut._count > 0)
		{
			...
			if (escapeRange > 0)
			{
				...
				if (skill.getTargetType(this) == SkillTargetType.TARGET_ONE)
				{
					...
					targetList.add((CopyOfL2Character) targets[0]);
				}
				else
				{
					...
					for (L2Object target : getKnownList().getKnownCharacters())
					{
						if (target instanceof CopyOfL2Character)
						{
							...
							if (this instanceof L2Playable)
							{
								...
								else if (!canAttackDueToSoloMob((CopyOfL2Character) target))
									continue;
							}
							if (!((CopyOfL2Character) target).isInsideRadius(mut.x, mut.y, mut.z, escapeRange, false, false))
								continue;
							if (skill.isOffensive() && !skill.isNeutral())
							{
								if (this instanceof L2PcInstance)
		...
		try
		{
			...
			for (L2Object tgt : targets)
			{
				...
				{
					CopyOfL2Character target = (CopyOfL2Character) tgt;
					/*
					...
					if (skill.isHeal())
					{
						if (this instanceof L2PcInstance)
						{
							if (getActingPlayer().isInGludin())
								...
								mpConsume *= 6;
							else if (getActingPlayer().isInsideZone(CopyOfL2Character.ZONE_PVP))
								mpConsume *= 3.5;
							else
								mpConsume *= 2;
							mpConsume *= 1.3;
	...
	public void onMagicFinalizer(MagicUseTask mut)
	{
		...
		else
		{
			if (this instanceof L2Playable)
			{
				...
				{
					if (target != null && target instanceof CopyOfL2Character && target != this && getTarget() != null && target == getTarget())
					{
						if (target.isAutoAttackable(this))
	...
	public final void callSkill(L2Skill skill, L2Object[] targets)
	{
		try
		{
			...
			{
				if (trg instanceof CopyOfL2Character)
				{
					// Set some values inside target's instance for later use
					final CopyOfL2Character target = (CopyOfL2Character) trg;
			...
			if (skill.getMustNegateId() >= 1)
			{
				...
				{
					if (targets[0] instanceof CopyOfL2Character)
					{
						for (L2Effect e : ((CopyOfL2Character) targets[0]).getAllEffects())
						{
							if (e != null && e.getSkill() != null && e.getSkill().getId() == id)
								e.exit();
						}
	...
	{
		return isBehind(target, calcStat(Stats.BACK_ANGLE_INCREASE, 50, (CopyOfL2Character) target, null));
	}
	...
	public boolean isBehind(L2Object target, double angle)
	{
		...
		if (target instanceof CopyOfL2Character)
		{
			CopyOfL2Character target1 = (CopyOfL2Character) target;
	 ...
	 * <BR>
	 */
	public boolean isInFrontOf(CopyOfL2Character target)
	{
		if (target == null)
			return false;
	 ...
	 * <BR>
	 */
	public final int getRandomDamage(CopyOfL2Character target)
	{
	...
	// Property - Public
	// EXP REWARD HERE
	public final double calcStat(Stats stat, double init, CopyOfL2Character target, L2Skill skill)
	{
	...
	// Property - Public
	public int getAccuracy(CopyOfL2Character target)
	{
	...
	public double getCriticalDmg(CopyOfL2Character target, double init, L2Skill skill)
	{
		return getStat().getCriticalDmg(target, init, skill);
	}
	
	public int getCriticalHit(CopyOfL2Character target, L2Skill skill)
	{
		return getStat().getCriticalHitRate(target, skill);
	}
	
	public int getEvasionRate(CopyOfL2Character target)
	{
		return getStat().getEvasionRate(target);
	}
	
	public int getVampRate(CopyOfL2Character target)
	{
	...
	public int getMAtk(CopyOfL2Character target, L2Skill skill)
	{
	...
	public int getMCriticalHit(CopyOfL2Character target, L2Skill skill)
	{
		return getStat().getMCriticalHit(target, skill);
	}
	
	public int getMDef(CopyOfL2Character target, L2Skill skill)
	{
	...
	public int getPAtk(CopyOfL2Character target)
	{
		return getStat().getPAtk(target);
	}
	
	public double getPAtkAnimals(CopyOfL2Character target)
	{
		return getStat().getPAtkAnimals(target);
	}
	
	public double getPAtkDragons(CopyOfL2Character target)
	{
		return getStat().getPAtkDragons(target);
	}
	
	public double getPAtkInsects(CopyOfL2Character target)
	{
		return getStat().getPAtkInsects(target);
	}
	
	public double getPAtkMonsters(CopyOfL2Character target)
	{
		return getStat().getPAtkMonsters(target);
	}
	
	public double getPAtkPlants(CopyOfL2Character target)
	{
		return getStat().getPAtkPlants(target);
	}
	
	public double getPAtkGiants(CopyOfL2Character target)
	{
		return getStat().getPAtkGiants(target);
	}
	
	public double getPAtkUndead(CopyOfL2Character target)
	{
		return getStat().getPAtkUndead(target);
	}
	
	public double getPAtkDemons(CopyOfL2Character target)
	{
		return getStat().getPAtkDemons(target);
	}
	
	public double getPAtkMCreatures(CopyOfL2Character target)
	{
		return getStat().getPAtkMCreatures(target);
	}
	
	public double getPDefAnimals(CopyOfL2Character target)
	{
		return getStat().getPDefAnimals(target);
	}
	
	public double getPDefDragons(CopyOfL2Character target)
	{
		return getStat().getPDefDragons(target);
	}
	
	public double getPDefInsects(CopyOfL2Character target)
	{
		return getStat().getPDefInsects(target);
	}
	
	public double getPDefMonsters(CopyOfL2Character target)
	{
		return getStat().getPDefMonsters(target);
	}
	
	public double getPDefPlants(CopyOfL2Character target)
	{
		return getStat().getPDefPlants(target);
	}
	
	public double getPDefGiants(CopyOfL2Character target)
	{
	...
	public int getPDef(CopyOfL2Character target)
	{
	...
	// Method - Public
	public void addStatusListener(CopyOfL2Character object)
	{
		getStatus().addStatusListener(object);
	}
	
	public void reduceCurrentHp(double i, CopyOfL2Character attacker, L2Skill skill)
	{
	 ...
	 * }
	 */
	public void reduceCurrentHpByDOT(double i, CopyOfL2Character attacker, L2Skill skill)
	{
		reduceCurrentHp(i, attacker, !skill.isToggle(), true, skill);
	}
	
	public void reduceCurrentHp(double value, CopyOfL2Character attacker, boolean awake, boolean isDOT, L2Skill skill, boolean bypassCP)
	{
	...
	public void reduceCurrentHp(double i, CopyOfL2Character attacker, boolean awake, boolean isDOT, L2Skill skill)
	{
	...
	public void removeStatusListener(CopyOfL2Character object)
	{
	 ...
	 * <BR>
	 */
	public void sendDamageMessage(CopyOfL2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{}
	
	public void sendDamageMessage(CopyOfL2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss, boolean extra)
	{}
	...
	{
		private final CopyOfL2Character	_activeChar;
		private final L2Skill		_skill;
		
		UsePotionTask(CopyOfL2Character activeChar, L2Skill skill)
		{
	...
	final public boolean CanAttackDueToInEvent(CopyOfL2Character target)
	{
		if (!(target instanceof L2Playable))
			return true;
		if (!(this instanceof L2Playable))
	...
	public boolean isInSameMobGroupAs(CopyOfL2Character obj)
	{
		if (obj == null)
			return false;
