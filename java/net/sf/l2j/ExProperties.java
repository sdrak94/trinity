...
public class ExProperties extends Properties
{
	...
	public Map<Integer, Integer> getPropertyMapII(final String name, HashMap<Integer, Integer> defaultValue)
	{
		Map<Integer, Integer> val = defaultValue;
		final String value;
		if ((value = super.getProperty(name, null)) != null && value.length() != 0)
		{
			final String[] values = value.split(defaultDelimiter);
			val = new HashMap<>(values.length);
			for (int i = 0; i < values.length; i += 2)
				val.put(Integer.valueOf(values[i]), Integer.valueOf(values[i + 1]));
		}
		return val;
	}
